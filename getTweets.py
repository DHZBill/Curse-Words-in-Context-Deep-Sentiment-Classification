...
with open('trainng_data_twitter.csv', 'w', newline='') as file:
    ...
    for hashtagIndex in range(0, len(allHashtags)):
        for curseword in cursewords:
          try:
            getTweets("?q=%23" + allHashtags[hashtagIndex] + "%20" + curseword +"&lang=en&tweet_mode=extended&count="+str(kCountPerRequest), curseword, hashtagIndex, writer)
