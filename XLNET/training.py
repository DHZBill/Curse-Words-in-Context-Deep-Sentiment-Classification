...
PATH_CSV_TRAIN = "unique_tweets_new.csv"
...
def preprocess(df):
    ...
    data = data.drop(data.query('sentiment == 0').sample(frac=0.3).index)
    data = data.drop(data.query('sentiment == 2').sample(frac=0.3).index)
